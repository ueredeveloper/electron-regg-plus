...
class AnalyseView {
	...
	//classes: inter-sistema inter-bh inter-uh
	...
	render() {
		let innerHTML = `
			...
			<p></p>
			<p>3. Outorga anterior: <span class="inter-despacho-tipo">Despacho</span>/SRH nº
			<span class="inter-despacho-numero"></span>, de <span class="inter-despacho-data"></span>.
			</p>
			<p></p>
            <p><br></p>
			<p>4. Trata o presente processo de renovação de outorga de direito de uso de água subterrânea
			por meio de 01 (um) poço <span class="inter-tipo-poco"></span>, para fins de
			<span class="inter-finalidades"></span> e demanda total estimada de <span class="dem-l-dia"></span> L/dia.
			A outorga anterior foi concedida por meio do <span class="inter-despacho-tipo">Despacho</span>/SRH nº
			<span class="inter-despacho-numero"></span>, de <span class="inter-despacho-data"></span>,
			publicado no Diário Oficial do Distrito Federal nº <span class="inter-dodf-numero"></span>,
			de <span class="inter-dodf-data"></span>, com validade de
			<span class="inter-validade-anterior"></span> anos, sendo automaticamente prorrogada por igual período,
			conforme o disposto na Resolução nº 07, de 22 de maio de 2019, uma vez que não houve solicitação de
			alteração da demanda outorgada. Assim, o ato de outorga permanece válido até dia
			<span class="inter-validade-expiracao"></span>. Destaca-se que o empreendimento é atendido pela
			Companhia de Saneamento Ambiental do Distrito Federal (CAESB). Dessa forma, o pedido de renovação
			da outorga será analisado neste parecer.
			</p>
			<p></p>
            <p><br></p>
			...
			<p>Figura 03: Croqui da área com existência de irrigação em 31/05/2016.</p>
			<p><br><br></p>
			...
			<p>14. A documentação solicitada foi atendida e sustenta a solicitação de renovação de outorga de direito de uso de água
			subterrânea.</p>
	...
	update(documento, interferencia) {
		...
		Array.from(_____items).forEach(element => {
			element.innerHTML = new InterferenciaModel().getUnidadeHidrografica(interferencia) || 'XXX';
		});
